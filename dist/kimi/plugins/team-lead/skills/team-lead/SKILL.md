...
Model tiers are named here rather than pinned because Kimi Code assigns the model per sub-agent — map the roles to the cheapest available tiers when configuring your harness. Pick the cheapest role that will finish in one or two attempts. Escalate one tier when a dev gets stuck (see below). Never start at the top tier to "be safe".
...
3. Run `$KIMI_PLUGIN_ROOT/skills/team-lead/scripts/rebase-worktrees.sh --base <branch> --push` (invoke via `$KIMI_PLUGIN_ROOT`, never a hardcoded path): it rebases each worktree that has an open PR to that base, force-pushes clean ones, and leaves conflicted ones mid-rebase. Dirty worktrees and other authors' PRs are skipped and listed.
