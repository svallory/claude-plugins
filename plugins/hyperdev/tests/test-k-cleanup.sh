#!/usr/bin/env bash
# Row K — cleanup (hyperdev-cleanup.sh), the one script that deletes. List
...
CLEANUP="$SCRIPTS_DIR/hyperdev-cleanup.sh"
...
touch "$d/.hyperdev-convert.preflight"
touch "$d/.claude/MEMORY.md.hyperdev-orig"
...
assert_ok "preflight file gone" test ! -e "$d/.hyperdev-convert.preflight"
assert_ok "memory backup gone" test ! -e "$d/.claude/MEMORY.md.hyperdev-orig"
