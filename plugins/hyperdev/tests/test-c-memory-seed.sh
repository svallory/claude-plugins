...
idx="$d/.hyperdev/memory/MEMORY.md"
assert_ok "memory file written" test -f "$d/.hyperdev/memory/hyperdev-layout.md"
...
assert_eq "no .tmp litter left behind" \
  0 "$(find "$d/.hyperdev" -name '*.tmp' | wc -l | tr -d ' ')"
...
# off by this harness.
printf -- '- [Space layout](hyperdev-layout.md) — old entry\n' > "$idx"
(
  set -euo pipefail
  source "$SCRIPTS_DIR/hyperdev-lib.sh"
  write_memory_seed "$d" proj
...
# ends in /.hyperdev/memory, with the directory itself created.
# ---------------------------------------------------------------------------
...
assert_ok "scaffold creates .hyperdev/memory/" test -d "$d2/.hyperdev/memory"
val="$(json_key "$s2" autoMemoryDirectory)"
case "$val" in
  /*) pass "autoMemoryDirectory is absolute" ;;
  *)  fail "autoMemoryDirectory is absolute (got '$val')" ;;
esac
assert_eq "autoMemoryDirectory ends in /.hyperdev/memory" \
  "$d2/.hyperdev/memory" "$val"
...
assert_eq "merge sets autoMemoryDirectory" \
  "$d3/.hyperdev/memory" "$(json_key "$s3" autoMemoryDirectory)"
