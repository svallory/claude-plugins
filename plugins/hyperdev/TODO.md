# TODO — hyperdev improvement backlog
...
- **Multi-stack awareness.** A monorepo with TS + Go should surface tools
  ...
  intersection higher.
- **Interactive flow** in `/hyperdev:tools`:
...
- [x] `/hyperdev:help` command (orientation + standard flow)
...
- [ ] Reference `.hyperdev/memory/MEMORY.md` from `AGENTS.md` so Codex
      sessions get space memory too (Codex has no autoMemoryDirectory).
- [ ] Thin Codex config shim reusing `hyperdev-check.sh` unchanged (the
...
- [ ] Surface check-hook overhead: measure and report the fixed per-edit
      cost during /hyperdev:tools setup so the user chooses commands
