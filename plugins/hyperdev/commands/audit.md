...
```bash
bash "${CLAUDE_PLUGIN_ROOT}/scripts/hyperdev-audit.sh" [path]
```

It reports, grouped by severity (problems / warnings / info):

- layout drift — missing scaffold directories, missing `HYPERDEV.md`
...
1. **Layout gaps** — point at `/hyperdev:adopt --apply`, which scaffolds
