---
name: plan-develop
description: Use when running the Develop phase of hyper plan — executing tasks.md one task at a time, validating each against define.md criteria and design.md constraints before marking it complete. Triggers on "develop the feature", "work the tasks", "/hyperdev:plan <feature> develop", or a spec dir that already has tasks.md.
---
...
2. **Run the project's checks.** If `.claude/hyperdev.json` has an enabled
   `check` config (see the `hyperdev-tooling` skill), run that command; the
