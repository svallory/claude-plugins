...
HyperDev's *Deterministic First* principle: **if it can be done
...
engine** — the command instructs the agent, and the `hyperdev-gen` skill holds
...
Read the **hyperdev-gen** skill before generating anything. Its rules are not
optional.
...
```
/hyperdev:gen                          # list available templates
/hyperdev:gen react-component          # generate; dest asked or derived
/hyperdev:gen react-component src/features/billing
```
...
6. **Run the project's checks** (*Real-time Feedback*): if
   `.claude/hyperdev.json` has a `check`/`checks` config, run it against the
