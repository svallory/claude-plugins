...
#
# Config (project .claude/hyperdev.json) — single check:
#   {
#     "check": {
#       "enabled": true,
...
while IFS='|' read -r tag run_b64 cmd_b64 tmo exts_b64; do
  ...
  if [[ -z "$cmd" && -n "$run" ]]; then
    if [[ $pm_run_resolved -eq 0 ]]; then
      pm_run="$(bash "$(dirname "${BASH_SOURCE[0]}")/hyperdev-stack.sh" detect "$root" </dev/null 2>/dev/null \
                | sed -n 's/^PM_RUN=//p')"
      pm_run_resolved=1
    fi
  ...
  if [[ -n "$timeout_bin" && $status -eq 124 ]]; then
    echo "hyperdev: check timed out after ${tmo}s: $cmd" >&2
    exit 2
  fi
  ...
  if [[ $status -eq 126 || $status -eq 127 ]]; then
    seen_env="${TMPDIR:-/tmp}/hyperdev-check-envfail-${session_id:-$PPID}"
    if ! grep -qxF "$cmd" "$seen_env" 2>/dev/null; then
      printf '%s\n' "$cmd" >>"$seen_env" 2>/dev/null || true
      {
        echo "hyperdev: check command not runnable (exit $status): $cmd"
        echo "$out" | tail -5
