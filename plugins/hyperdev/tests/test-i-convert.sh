#!/usr/bin/env bash
# Row I — checkout conversion (hyperdev-adopt.sh case B). Adopting an ordinary
...
ADOPT="$SCRIPTS_DIR/hyperdev-adopt.sh"
...
assert_contains "HYPERDEV.md left untouched on re-adopt" "$out2" \
  "exists   HYPERDEV.md (left untouched)"
...
touch "$d/HYPERDEV.md"                       # untracked marker (old layout)
mkdir -p "$d/.claude"
...
assert_contains "HYPERDEV.md regenerated for the bare layout" \
  "$(cat "$d/HYPERDEV.md")" "bare layout"
