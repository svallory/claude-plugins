---
name: hyperdev-gen
description: Use when generating files from a project template — the rules for template-driven generation where everything outside marked prompt regions is copied verbatim. Triggers on "generate from template", "hyperdev gen", "scaffold a component", "create from the template", "new file like the others".
---
...
- **Project checks**: run the `check`/`checks` config from
  `.claude/hyperdev.json` when present (see `/hyperdev:tools`). Failures in
