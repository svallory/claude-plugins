...
# a report for a human (or /hyperdev:audit) to act on, not a gate.
# Usage: hyperdev-audit.sh [space-root]
...
if [[ -z "$root" ]]; then
  root="$(find_space_root "$PWD")" || {
    ...
    echo "pass a path explicitly, or use hyperdev-adopt.sh to opt a repo in" >&2
    exit 0
  }
fi
root="$(cd "$root" && pwd)"
