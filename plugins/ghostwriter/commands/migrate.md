...
```bash
/ghostwriter:migrate              # Migrate and move files
/ghostwriter:migrate --dry-run    # Show what would change without moving anything
```
...
If `.ghostwriter/` doesn't exist at all, report: "No ghostwriter setup found. Run `/ghostwriter:setup` instead." and STOP.
