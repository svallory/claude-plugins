...
2. **The actual codebase.** Explore the worktree: existing architecture,
   ...
   hypothetical ones.
3. The project's toolchain (`.claude/hyperdev.json` if present) — the checks
...
next step is `/hyperdev:plan <feature> decompose` (the `plan-decompose`
skill).
