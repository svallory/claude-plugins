#!/usr/bin/env bash
# Row D — the PostToolUse check hook (hyperdev-check.sh), driven exactly the
...
HOOK="$SCRIPTS_DIR/hyperdev-check.sh"
ERR="$FIX/hook-stderr"
...
p="$(new_proj d2)"
cat > "$p/.claude/hyperdev.json" <<'EOF'
...
p="$(new_proj d8)"
cat > "$p/.claude/hyperdev.json" <<'EOF'
{ "check": { "enabled": true, "command": "/nonexistent/hyperdev-test-cmd-xyzzy" } }
EOF
run_hook "$p/src/app.ts"
...
p="$(new_proj d9)"
echo '{ this is not json' > "$p/.claude/hyperdev.json"
run_hook "$p/src/app.ts"
