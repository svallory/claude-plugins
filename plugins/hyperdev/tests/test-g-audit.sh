#!/usr/bin/env bash
# Row G — the read-only audit (hyperdev-audit.sh): a healthy space reports no
...
AUDIT="$SCRIPTS_DIR/hyperdev-audit.sh"
...
mkdir -p "$d/data" "$d/notes" "$d/scratch" "$d/bin"
touch "$d/HYPERDEV.md"
echo 'API_KEY=HYPERDEV_TEST_SECRET_VALUE_XYZZY' > "$d/.env"
out="$(bash "$AUDIT" "$d" 2>&1)"
assert_contains "the .env path is reported" "$out" ".env"
assert_not_contains "the .env contents never appear" "$out" "HYPERDEV_TEST_SECRET_VALUE_XYZZY"
