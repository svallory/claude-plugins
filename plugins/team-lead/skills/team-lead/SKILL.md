...
2. If the problem is capability, escalate one tier (haiku -> sonnet -> opus) with the failed attempts included in the brief so the next dev does not repeat them.
...
A waiter firing, an idle status, or the word "done" is a claim. Run this before believing it:
...
3. Run `scripts/rebase-worktrees.sh --base <branch> --push` from the skill directory: it rebases each worktree that has an open PR to that base, force-pushes clean ones, and leaves conflicted ones mid-rebase. Dirty worktrees and other authors' PRs are skipped and listed.
...
2. **Check its state**: idle/working/blocked, and whether a prompt sits unsent in its input box.
...
Parallel devs in one repo share the local DB/ports. Before asking several devs to run the app at once, serialize app-running work (one dev at a time) or give each a distinct DB name/port in the brief; otherwise they collide on migrations and stall on the 3-attempt rule.
