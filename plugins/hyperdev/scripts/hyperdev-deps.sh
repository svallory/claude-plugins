...
#
# This implements HyperDev's "Engineered Friction" principle: adding a
...
#
# It is OFF unless the project opts in via .claude/hyperdev.json:
#   { "deps": { "enabled": true } }
#
...
seen_file="${TMPDIR:-/tmp}/hyperdev-deps-${session_id:-$PPID}"
...
echo "hyperdev: new dependency $unseen added to $base. State in one sentence why an existing dependency or stdlib cannot do this, then continue. If it cannot be justified, remove it." >&2
exit 2
