...
If [okflint](#preferred-okflint-when-available) is installed and the bundle has a manifest, `okflint index --manifest okf-base.yaml --apply` generates conformant `index.md` files automatically (omit `--apply` for a dry-run diff).
...
[okflint](https://github.com/mattdav/okflint) is a deterministic, LLM-free linter for OKF bundles — 18 rules across 3 tiers (OKF core, manifest profile, hygiene). If installed, always prefer it over the built-in bash script.

**Agent behavior:** Before validating, check if okflint is installed (`command -v okflint`). If NOT installed, ask the user:

> "okflint (a dedicated OKF linter with manifest-driven profiles, wikilink resolution, and CI-friendly exit codes) is not installed. Want me to install it? Options:
> 1. `uv tool install okflint` (recommended, isolated)
> 2. `pip install okflint`
> 3. Skip it — fall back to basic validation with the bundled bash script"
...
```bash
# Full validation with a manifest — okf-base.yaml declares your types,
# required fields, and status vocabulary (exit 0 = pass, 1 = failure)
okflint validate --manifest okf-base.yaml ./bundle/

# No manifest: core-only diagnostics (spec rules plus reserved-file
# structure). Always exits 0 — observation, not a gate
okflint audit --bundle ./bundle --vault ./bundle

# Machine-readable output for CI pipelines
okflint validate --manifest okf-base.yaml --json ./bundle/
```

`okflint validate` exit codes: `0` = pass, `1` = conformance failure, `2` = bad or unreadable manifest.

**okflint advantages over the built-in script:**
- Manifest-driven profiles (`okf-base.yaml`): enforce custom required fields, status vocabularies, per-type constraints
- Wikilink resolution against a full Obsidian vault (`--vault`), so links outside the bundle don't false-positive
- JSON output (`--json`) for CI pipeline parsing
- Detects broken markdown links and ambiguous wikilinks
- `okflint index` generates spec-conformant `index.md` files (dry-run by default, `--apply` to write)
...
`kcmd` is a bidirectional sync tool between OKF-like local metadata and Knowledge Catalog. Think "git for metadata." Distributed as a TypeScript library and standalone CLI (`npm install kcmd`), and as an MCP server.
