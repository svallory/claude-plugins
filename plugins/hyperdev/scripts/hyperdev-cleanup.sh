...
# Same classification hyperdev-audit.sh uses for worktrees/ entries: live,
...
backup_file="$root/.claude/MEMORY.md.hyperdev-orig"
...
if [[ ${#delete_ids[@]} -eq 0 ]]; then
  ...
  [[ -f "$backup_file" ]] \
    && cand "backup:memory-index" "$(size_of "$backup_file")" ".claude/MEMORY.md.hyperdev-orig — pre-conversion memory index backup"

  [[ -f "$preflight_file" ]] \
    && cand "evidence:preflight" "$(size_of "$preflight_file")" ".hyperdev-convert.preflight — conversion capture; reconcile before deleting"
  ...
  case "$class" in
    ...
    backup)
      ...
      if [[ ! -f "$backup_file" ]]; then
        refuse "$id" "not present (.claude/MEMORY.md.hyperdev-orig)"
        continue
      fi
    ...
    evidence)
      ...
      if [[ ! -f "$preflight_file" ]]; then
        refuse "$id" "not present (.hyperdev-convert.preflight)"
        continue
      fi
