#!/usr/bin/env bash
# Adopt a repository into the hyperdev space layout.
# Usage: hyperdev-adopt.sh [path] [--apply]
#
...
report_loose_entries() {
  ...
  while IFS= read -r entry; do
    ...
    # /hyperdev:gen keeps templates at <space>/templates/, so that directory
...
[[ -e "$staging" ]] \
  && refusals+=(".hyperdev-convert already exists — a previous conversion did not finish; resolve it first")
...
while IFS= read -r entry; do
  ...
  case "$base" in
    .git) continue ;;
    .hyperdev-convert) continue ;;   # refused above; never classify it
    .claude) claude_split=1; continue ;;
    HYPERDEV.md)
      if git -C "$root" ls-files --error-unmatch HYPERDEV.md >/dev/null 2>&1; then
...
preflight_file="$root/.hyperdev-convert.preflight"
{
  echo "# hyperdev-adopt pre-conversion capture: $root"
  echo "# refs"
  printf '%s\n' "$pre_refs"
...
pre_norm="$pre_state"
for k in "${keep_space[@]}" ".claude/memory/hyperdev-layout.md"; do
  pre_norm="$(awk -v k="$k" 'substr($0, 4, length(k)) != k' <<< "$pre_norm")"
done
if [[ "$pre_norm" != "$post_state" ]]; then
  note=""
  [[ $memory_stripped -eq 1 ]] \
    && note="(.claude/MEMORY.md had its hyperdev-layout line moved to the space root, which can account for a MEMORY.md difference.)"
